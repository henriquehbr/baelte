...
const npmInstall: _CustomActionFunction = (answers, config) =>
      ...
      dependencyNames?.on('close', code => {
        switch (dependencyType) {
          case 'dependencies':
            didSucceed(code)
              ? resolve(
                  `${config?.packageManager} installed ${config?.dependencies.length} dependencies`
                )
          ...
          case 'devDependencies':
            didSucceed(code)
              ? resolve(
                  `${config?.packageManager} installed ${config?.devDependencies.length} devDependencies`
                )
          ...
          case 'peerDependencies':
            didSucceed(code)
              ? resolve(
                  `${config?.packageManager} installed ${config?.peerDependencies.length} peerDependencies`
                )
              : reject(
                  `${config?.packageManager} couldn’t install peerDependencies, error: ${code}`
                )
            break
        }
      })
